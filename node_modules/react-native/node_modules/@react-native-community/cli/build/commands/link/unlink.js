...
const unlinkDependency = (platforms, project, dependency, packageName, otherDependencies) => {
  Object.keys(platforms || {}).forEach(platform => {
    ...
    projectConfig, packageName, // $FlowFixMe
    dependencyConfig);
    ...
    linkConfig.unregister(packageName, // $FlowFixMe
    dependencyConfig, // $FlowFixMe
    projectConfig, otherDependencies);
...
async function unlink(args, ctx, opts) {
  ...
  try {
    if (dependency.hooks.preulink) {
      await (0, _makeHook.default)(dependency.hooks.preulink)();
    }
  ...
  Object.keys(platforms || {}).forEach(platform => {
    ...
    _cliTools().logger.info(`Unlinking assets from ${platform} project`); // $FlowFixMe
